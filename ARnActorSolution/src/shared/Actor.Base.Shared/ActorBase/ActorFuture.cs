...
namespace Actor.Base
{
    public class Future<T> : BaseActor, IFuture<T>
    {
        ...
        public T Result() => (T)AsyncReceive(t => t is T).Result;

        public T Result(int timeOutMS) => (T)Receive(t => t is T, timeOutMS).Result;

        public async Task<T> ResultAsync() => (T)await AsyncReceive(t => t is T).ConfigureAwait(false);

        public async Task<T> ResultAsync(int timeOutMS) => (T)await Receive(t => t is T, timeOutMS).ConfigureAwait(false);

        public T Result(Func<object, bool> aPattern) => (T)AsyncReceive(aPattern).Result;

        public async Task<T> ResultAsync(Func<object, bool> aPattern) => (T)await AsyncReceive(aPattern).ConfigureAwait(false);
    ...
    public class Future<T1,T2> : BaseActor, IFuture<T1, T2>
    {
        ...
        public IMessageParam<T1,T2> Result() => (IMessageParam<T1, T2>)AsyncReceive(t => t is IMessageParam<T1, T2>).Result;

        public IMessageParam<T1, T2> Result(int timeOutMS) => (IMessageParam<T1, T2>)Receive(t => t is IMessageParam<T1, T2>, timeOutMS).Result;

        public async Task<IMessageParam<T1, T2>> ResultAsync() => (IMessageParam<T1, T2>)await AsyncReceive(t => t is IMessageParam<T1, T2>).ConfigureAwait(false);
    ...
    public class Future<T1, T2, T3> : BaseActor, IFuture<T1, T2, T3>
    {
        ...
        public IMessageParam<T1, T2, T3> Result() => (IMessageParam<T1, T2, T3>)AsyncReceive(t => t is IMessageParam<T1, T2, T3>).Result;
        ...
        public async Task<IMessageParam<T1, T2, T3>> ResultAsync() => (IMessageParam<T1, T2, T3>)await AsyncReceive(t => t is IMessageParam<T1, T2, T3>).ConfigureAwait(false);
