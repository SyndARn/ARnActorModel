...
namespace Actor.Util
{
    public class FsmBehaviors<TState, TEvent> : Behaviors
    {
        ...
        internal TState GetCurrentState() => Current;

        internal void ChangeState(TState aState) => Current = aState;
    ...
    public class FsmBehavior<TState, TEvent> : Behavior<TEvent>
    {
        ...
        {
            bool result = (LinkedTo as FsmBehaviors<TState, TEvent> != null) && (LinkedTo as FsmBehaviors<TState, TEvent>).GetCurrentState().Equals(StartState) && (Condition == null || Condition(anEvent));
